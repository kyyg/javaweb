  ...
  <script>
    ...
    function fCheck() {
    	let fName = document.getElementById("file").value;
			...
			let maxSize = 1024 * 1024 * 5; // 업로드 가능파일은 5MByte까지로 한다.
			...
			if(ext != "JPG" && ext != "GIF" && ext != "PNG" && ext != "ZIP" && ext != "HWP" && ext != "PPT" && ext != "PPTX") {
				...
				alert("업로드할 파일의 최대 용량은 5MByte입니다.");
			}
			else {
				//alert("전송완료");
				myform.submit();
...
<p><br/></p>
<div class="container">
  <h2>파일 업로드 연습(멀티파일처리3)</h2>
  <p>COS라이브러리를 이용한 파일 업로드</p>
  <div>여러개의 파일을 선택하는 경우는 'Ctrl+클릭/Shift+클릭' 하세요</div>
  ...
  <hr/>
  <input tpye="button" value="다운로드사이트로이동" onclick="location.href='${ctp}/DownLoad.st';" class="btn btn-primary form-control" />
</div>
<p><br/></p>
